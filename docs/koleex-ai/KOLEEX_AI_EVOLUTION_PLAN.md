...
Duplex streaming, VAD, turn detection, barge-in, multilingual (EN/AR/EGY/ZH). Reuses the existing ElevenLabs TTS route (currently QA-only) where reachable. ~~**China gate:** realtime voice provider accessibility is **unverified**~~ — **RESOLVED.** The owner selected `qwen-audio-3.0-realtime-plus`, which is **mainland-hosted** and tops the Artificial Analysis Speech-to-Speech ranking, so the China gate that blocked this phase is closed. A new blocker took its place: the vendor's documented handshake carries the **real API key** and exposes **no ephemeral token**, so the browser cannot connect directly and Vercel cannot hold the socket — the phase needs a small relay service on a second host. Designed in full, with the measured 40 K context budget and the security boundary, in [`PHASE_15_VOICE_DESIGN.md`](./PHASE_15_VOICE_DESIGN.md). **Owner decision required before any code.** **Depends:** 3, 4, 5.
