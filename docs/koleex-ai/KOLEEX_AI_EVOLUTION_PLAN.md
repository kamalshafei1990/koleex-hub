...
| N8 | **The agent route keeps its own provider call** — and it CANNOT be closed by re-pointing it. Found in 3D analysis: the route's fast lane goes through `providers/deepseek.ts`, which is gated on **`USE_DEEPSEEK=true` in addition to the key** — a documented kill-switch. `core/transport.ts` and `ai-provider.ts` activate on the **key alone**. That asymmetry is deliberate and written down. Routing the lane through `chatWithTools()` would silently disable an operational control; making the adapter honour the flag would silently gate the agent loop instead. **The decision belongs to Phase 4's router**, where per-lane policy exists. Asserted as a count meanwhile so it cannot grow. | **4** (moved from 3) | medium — two transports, and the flag question is real |
