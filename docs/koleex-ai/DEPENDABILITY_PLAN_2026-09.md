...
| "still the voice has a glitch" (19:06 UTC call) | The model wrote the pictures' markdown into its spoken turn — "here are pictures of the factory. ! (exhibitorsearch.messefrankfurt.com) ! (…)" — and the voice read the punctuation and the host aloud. The call's search envelope carried picture URLs, a markdown rule and "cite the source URL": all wrong in a mouth. The same call then died at 19:06:45, right after that pictures answer — the third death of the day after pictures, before #405's mitigation was live | **Fixed** — on a call the model is handed the count of pictures already on the screen, a result's site name instead of its URL, and a note written for speech (`forVoice`); the beacon's event histogram now leads with cancellations and errors so the next cut sentence is diagnosable |
| "still too many problems, still a glitch" (2026-09-12 04:14 UTC call, socket lane via the relay) | Pictures asked for on a call never reached the screen since #406: the screen read them from the spoken envelope, which #406 had stripped of `images` (correctly, for the voice) — the model was told four pictures were showing, the owner saw none. And the relay ran in Amsterdam, not Singapore: China → Europe → vendor → Europe → China for every audio frame on this lane | **Fixed** — the route sends `pictures` beside the envelope and the screen reads that; `railway.json` pins the relay to Singapore. Open: whether the audio glitch is fully gone on the Singapore path needs the owner's next call (and B7's real-device session for anything left) |
