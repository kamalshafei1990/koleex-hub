...
import { draftType, insertDraft, periodReport, startReportDraft } from "@/lib/server/reports/drafts";
import {
  REPORT_LIST_COLS, isUuid, listPeople, loadOrgTree, requireReportsUser,
...
export async function POST(req: Request) {
  ...
  if (body?.request !== undefined) {
    const found = await draftType(auth.tenant_id, body?.template_key);
    if (!found) return NextResponse.json({ error: "unknown_template" }, { status: 400 });
    const { tpl } = found;
    ...
    const id = await insertDraft(auth, tpl, { start: day, end: day, key: periodKey }, "", Object.entries(facts).map(([sid, text]) => ({ id: sid, text })));
  ...
  /* Everything else: the Write button's rules, shared with Koleex AI's
     startReportDraft (lib/server/reports/drafts). */
  const date = typeof body?.date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(body.date) ? body.date : new Date().toISOString().slice(0, 10);
  const done = await startReportDraft(auth, { templateKey: body?.template_key, date, title: body?.title });
  if (done === "unknown_template") return NextResponse.json({ error: "unknown_template" }, { status: 400 });
  if (done === "request_only") return NextResponse.json({ error: "request_only" }, { status: 403 });
  if (done === "forbidden") return NextResponse.json({ error: "forbidden" }, { status: 403 });
  if (done === "hidden") return NextResponse.json({ error: "hidden" }, { status: 403 });
  if (done === "failed") return NextResponse.json({ error: "Could not create the report." }, { status: 500 });
  return NextResponse.json({ id: done.id, existing: done.existing }, { status: done.existing ? 200 : 201 });
}
