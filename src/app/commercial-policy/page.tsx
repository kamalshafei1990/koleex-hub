...
function PolicyHealthStrip({ s }: { s: CommercialPolicySnapshot }) {
  ...
  const fxFreshLabel = (() => {
    if (!updatedAt) return "FX: not yet updated";
    const fx = Number(s.settings?.fx_cny_per_usd ?? 0).toFixed(4);
    return `FX ${fx} · updated ${relativeDay(updatedAt)}`;
  })();
  const fxStale = (() => {
    if (!updatedAt) return true;
    const ageMs = nowMs() - updatedAt.getTime();
    return ageMs > 1000 * 60 * 60 * 36; // >36h since last write
  })();
  ...
  return (
    <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)] px-4 py-3 flex flex-wrap items-center gap-x-5 gap-y-2">
      <HealthPill
        ...
        hint={engineOn ? "Lines price from the policy" : "Prices are entered manually until you turn it on in Settings"}
      />
      <HealthPill
        ok={!fxStale}
        label={fxFreshLabel}
        hint={fxStale ? "Refresh the rate in Settings → FX" : "Auto-updates daily"}
      />
    </div>
  );
}
...
function HealthPill({ ok, label, hint }: { ok: boolean; label: string; hint?: string }) {
  return (
    <div className="flex items-center gap-2 min-w-0">
      ...
      <div className="min-w-0">
        <div className="text-[12px] font-medium text-[var(--text-secondary)] truncate">{label}</div>
        {hint && <div className="text-[10px] text-[var(--text-dim)] truncate">{hint}</div>}
      </div>
    </div>
  );
}

/* Small date helpers — kept local so they can't drift. */
