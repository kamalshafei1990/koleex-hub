...
/* PhoneIcon — SEMANTIC icon: "phone". Registry-first since 2026-08-07
   (field.phone): every call site follows what the Visual Library binds. Inline
...
const PhoneIcon = forwardRef<HTMLSpanElement, { size?: number | string; className?: string; style?: React.CSSProperties }>(
  ({ size = 24, className, style }, ref) => {
    ...
    return (
      <span ref={ref} aria-hidden className={`inline-flex shrink-0 ${className ?? ""}`} style={sized}>
        <BoundIcon semanticKey="field.phone" className="h-full w-full" fallback={fallbackFor(s)} />
      </span>
    );
  },
);
