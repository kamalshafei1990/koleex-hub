...
/* DocumentIcon — SEMANTIC icon: "document / file". Registry-first since 2026-08-07
   (entity.document): every call site follows what the Visual Library binds. Inline
...
const DocumentIcon = forwardRef<HTMLSpanElement, { size?: number | string; className?: string; style?: React.CSSProperties }>(
  ({ size = 24, className, style }, ref) => {
    ...
    return (
      <span ref={ref} aria-hidden className={`inline-flex shrink-0 ${className ?? ""}`} style={sized}>
        <BoundIcon semanticKey="entity.document" className="h-full w-full" fallback={fallbackFor(s)} />
      </span>
    );
  },
);
