...
export default function Modal({ open, onClose, title, subtitle, children, footer, width = "max-w-xl" }: Props) {
  ...
  return (
    ...
    >
      <div role="dialog" aria-modal="true" aria-label={title} className={`${width} kx-app kx-glass-pop kx-pop-in ${closing ? "kx-pop-closing" : ""} relative w-full bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded-2xl shadow-2xl shadow-black/40 flex flex-col max-h-[85vh]`}>
        {/* Header */}
