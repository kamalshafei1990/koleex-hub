  ...
  return (
    <div className="flex items-center gap-3 min-w-0 flex-1">
      <input
        ...
        aria-valuetext={value == null ? "Not assessed" : `${v} — ${level}`}
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div
        ...
        onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Add behavior indicator"
      >
        <div className="p-4 border-b border-[var(--border-subtle)] space-y-2.5">
          <div className="flex items-center justify-between">
            <h3 className="text-[14px] font-bold text-[var(--text-primary)]">Add Behavior Indicator</h3>
          ...
          <div className="relative">
            ...
            <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search indicators…"
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {list.length === 0 ? (
            <p className="px-3 py-8 text-center text-[12.5px] text-[var(--text-faint)]">No matching indicators.</p>
          ) : list.map((s) => (
            <button key={s.id} type="button" onClick={() => onPick(s.id)}
              ...
              <span className="flex items-center gap-1.5 min-w-0">
                ...
                {s.is_critical_default && <span className="shrink-0 rounded bg-rose-500/12 px-1 py-px text-[9px] font-bold uppercase text-rose-600 dark:text-rose-400">Critical</span>}
              </span>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)] shadow-2xl"
        onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Configure position behavior requirements">
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-subtle)]">
          <div>
            <h3 className="text-[14px] font-bold text-[var(--text-primary)]">Position Behavior Requirements</h3>
            <p className="text-[11px] text-[var(--text-dim)] mt-0.5">Applies to every employee in this position.</p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close" className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--text-dim)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)] transition-colors"><CrossIcon size={13} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            ...
            <p className="py-6 text-center text-[12.5px] text-[var(--text-faint)]">No requirements yet — add indicators from the library.</p>
          ) : reqs.map((r) => (
            <div key={r.behavior_indicator_id} className="flex flex-wrap items-center gap-2.5 rounded-xl border border-[var(--border-subtle)] px-3 py-2.5">
              <span className="min-w-[140px] flex-1 text-[13px] text-[var(--text-primary)] truncate">{indById.get(r.behavior_indicator_id)?.name ?? "Unknown"}</span>
              <label className="flex items-center gap-1.5 text-[10.5px] text-[var(--text-faint)]">Required
              ...
              <label className="flex items-center gap-1.5 text-[10.5px] text-[var(--text-faint)]">Weight
              ...
              <button type="button" onClick={() => upd(r.behavior_indicator_id, { is_critical: !r.is_critical })}
                ...
                {r.is_critical ? "Critical" : "Non-critical"}
              </button>
