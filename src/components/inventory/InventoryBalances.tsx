...
/* ---------------------------------------------------------------------------
   /inventory/balances — Derived stock balances per (inventory item, warehouse).
   Read-only. Now keyed on inventory_item_id (Phase O.2.1).
   --------------------------------------------------------------------------- */
          ...
          <table className="min-w-full text-[12.5px]">
            ...
            <tbody>
              {loading && rows.length === 0 ? (
                ...
                <tr><td colSpan={8} className="px-4 py-6 text-center text-[11px] text-gray-600">No balances to show.</td></tr>
              ) : (
                rows.map((r) => (
                  <tr key={r.id} className="border-b border-white/[0.03]">
                    ...
                    <td className="px-4 py-2 text-gray-300">{r.item_name ?? "—"}</td>
