...
export function HumanStatusPill({ status }: { status: string }) {
  ...
  const cls =
    status === "posted" || status === "active" || status === "received" || status === "completed" || status === "approved"
  ...
  return (
    <span className={`inline-flex items-center gap-1 h-[22px] px-2 rounded-full border text-[11px] font-semibold whitespace-nowrap ${cls}`}>
      {humanStatus(status)}
    </span>
  );
}
    ...
    >
      ...
      <div className="ml-auto flex flex-wrap items-center gap-1.5">
        ...
        <button
          type="button"
          onClick={onClear}
          className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface-subtle)] px-2.5 py-1.5 text-[11.5px] text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:border-[var(--border-focus)] transition-all"
        >
          {t("inv.bulk.clear")}
        </button>
      </div>
