...
export default function RelatedProductsSection({ related, onChange, currentProductId }: Props) {
  ...
  return (
    <div>
      <label className="block text-[12px] font-medium text-white/50 mb-3">Related Products</label>
      ...
      <div className="relative mb-4">
        ...
        />
        {results.length > 0 && (
          <div className="absolute z-20 top-full mt-1 w-full bg-[#1a1a1a] border border-white/[0.08] rounded-lg shadow-xl max-h-48 overflow-y-auto">
            {results.map(r => (
              <button
                key={r.id}
                onClick={() => add(r)}
                className="w-full text-left px-4 py-2.5 text-[13px] text-white/60 hover:bg-white/[0.06] hover:text-white transition-colors"
              >
                {r.product_name}
              </button>
            ))}
        ...
        {searching && <p className="absolute top-full mt-1 text-[11px] text-white/20">Searching...</p>}
      </div>

      {/* List */}
      {related.length === 0 ? (
        <p className="text-[13px] text-white/20 py-6 text-center border border-dashed border-white/[0.06] rounded-xl">No related products linked.</p>
      ) : (
        <div className="space-y-1.5">
          {related.map(r => (
            <div key={r.related_id} className="flex items-center justify-between h-10 px-4 rounded-lg bg-white/[0.03] border border-white/[0.06]">
              <div className="flex items-center gap-2 text-[13px] text-white/60">
                <LinkIcon className="h-3.5 w-3.5 text-white/20" />
                {r.related_name}
              </div>
              <button onClick={() => remove(r.related_id)} className="text-white/20 hover:text-red-400/70 transition-colors">
