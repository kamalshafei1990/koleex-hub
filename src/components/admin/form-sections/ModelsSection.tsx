...
function ModelCard({ model, idx, onUpdate, onRemove }: {
  ...
  const inp = "w-full h-10 px-4 rounded-lg bg-white/[0.05] border border-white/[0.08] text-[13px] text-white placeholder:text-white/25 outline-none focus:border-white/20";
  const lbl = "block text-[11px] font-medium text-white/40 mb-1";

  return (
    <div className="bg-white/[0.02] rounded-xl border border-white/[0.06] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 cursor-pointer" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-3">
          <span className="h-6 w-6 rounded-md bg-white/[0.06] flex items-center justify-center text-[11px] font-bold text-white/30">{idx + 1}</span>
          <span className="text-[14px] font-medium text-white/80">{model.model_name || "Untitled Model"}</span>
          {model.id && <span className="text-[10px] font-mono text-white/20">SKU: {model.slug}</span>}
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={(e) => { e.stopPropagation(); onRemove(); }} className="h-7 w-7 flex items-center justify-center rounded-md text-white/20 hover:text-red-400/70 transition-colors">
            <Trash2 className="h-3.5 w-3.5" />
          </button>
          {open ? <ChevronUp className="h-4 w-4 text-white/20" /> : <ChevronDown className="h-4 w-4 text-white/20" />}
        </div>
      </div>
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-white/[0.04] pt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={model.slug} onChange={(e) => onUpdate({ slug: e.target.value })} className={`${inp} font-mono text-white/50`} />
            </div>
          </div>
          <div>
          ...
          <p className="text-[10px] font-semibold uppercase tracking-wider text-white/20">Commercial (hidden from website)</p>
          <div className="grid grid-cols-2 gap-4">
            <div>
          ...
          <div className="h-px bg-white/[0.04] my-2" />
          <p className="text-[10px] font-semibold uppercase tracking-wider text-white/20">Pricing</p>
          ...
          <div className="h-px bg-white/[0.04] my-2" />
          <p className="text-[10px] font-semibold uppercase tracking-wider text-white/20">Packaging & Logistics</p>
          <div className="grid grid-cols-3 gap-4">
            <div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <label className="text-[12px] font-medium text-white/50">Product Models / Variants</label>
          <p className="text-[11px] text-white/20 mt-0.5">SKU is auto-generated on save</p>
        </div>
        <button
          onClick={addModel}
      ...
      {models.length === 0 ? (
        <p className="text-[13px] text-white/20 py-8 text-center border border-dashed border-white/[0.06] rounded-xl">No models yet. Add your first model variant.</p>
      ) : (
