...
interface Props {
  open: boolean;
  onClose: () => void;
  onCreated: (row: CategoryRow) => void;
  divisionId: string;
  divisionName: string;
  existingCount: number;
}

export default function CreateCategoryModal({ open, onClose, onCreated, divisionId, divisionName, existingCount }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(false);
    if (!row) { setError("Failed to create category. The slug might already exist."); return; }
    onCreated(row);
    reset();
    onClose();
  };
  ...
  return (
    <Modal
      ...
      subtitle={`Under division: ${divisionName}`}
      footer={
        <>
    ...
    >
      ...
      <div className="space-y-5">
        ...
        <div className="px-4 py-3 rounded-xl bg-[var(--bg-surface-subtle)] border border-[var(--border-subtle)]">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-ghost)] mb-1">Parent Division</p>
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Collaborative Robots"
            className={inp}
            autoFocus
          />
        </div>
