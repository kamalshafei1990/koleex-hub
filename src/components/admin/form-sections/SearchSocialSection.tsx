          ...
          <div className="rounded-lg overflow-hidden border border-[var(--border-subtle)]">
            <div className="aspect-[1.91/1] overflow-hidden">
              {customOg ? (
                ...
                <div className="w-full h-full bg-white flex items-stretch">
                  <div className="w-[42%] shrink-0 flex flex-col items-start pl-6 pr-3 py-4">
                    {/* logo near the top, small — nudged down a touch */}
                    <KoleexLogo className="h-2 w-auto text-black/90 shrink-0 block mt-1.5" />
                    ...
                    <div className="flex-1 min-h-0 min-w-0 flex flex-col justify-center">
                      <div className={`${name.length > 42 ? "text-[13px]" : name.length > 24 ? "text-[15px]" : "text-[17px]"} font-bold leading-snug text-black line-clamp-4`}>{name}</div>
                      {modelCode && (
                        <div className="text-[12px] font-mono tracking-wide text-black/55 mt-2 truncate">{modelCode}</div>
                      )}
                    </div>
                    {/* official footer */}
                  ...
                  <div className="flex-1 min-w-0 flex items-center justify-center p-3">
                    {primaryImageUrl ? (
                      ...
                      /* empty-photo fallback — a faint KOLEEX watermark so the
                         photo side never reads as a broken empty panel */
                      <KoleexLogo className="h-8 w-auto text-black/[0.07]" />
                    )}
                  </div>
                </div>
              )}
