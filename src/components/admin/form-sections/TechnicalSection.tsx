...
export default function TechnicalSection({ data, onChange, suggestions }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <ChipInput label="Colors" values={data.colors} onChange={(v) => onChange({ colors: v })} placeholder={suggestions?.colors?.length ? "Select or type color..." : "e.g. Silver (Enter to add)"} suggestions={suggestions?.colors} />
    </div>
  );
}
