      ...
      <div className="space-y-4">
        {latest ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <SummaryCard label="Behavior Score" value={latest.overall_behavior_score == null ? "—" : String(Math.round(latest.overall_behavior_score))} />
            <SummaryCard label="Position Match" value={latest.position_behavior_match == null ? "—" : `${Math.round(latest.position_behavior_match)}%`} />
            <SummaryCard label="Critical Gaps" value={String(latest.critical_gap_count ?? 0)} tone={(latest.critical_gap_count ?? 0) > 0 ? "bad" : "good"} />
            <SummaryCard label="Latest" value={latest.status === "finalized" ? "Finalized" : "Draft"} />
          </div>
        ) : (
          <p className="rounded-xl bg-[var(--bg-surface-subtle)]/60 px-4 py-6 text-center text-[12.5px] text-[var(--text-faint)]">
            No behavior assessments yet.
          </p>
        )}
            ...
            {history.slice(0, 8).map((h) => (
              <div key={h.id} className="flex items-center justify-between gap-3 px-3.5 py-2.5">
                <div className="min-w-0">
                  <span className="text-[12.5px] text-[var(--text-primary)] capitalize">{h.assessment_type.replace(/_/g, " ")}</span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${h.status === "finalized" ? "bg-emerald-500/12 text-emerald-600 dark:text-emerald-400" : "bg-[var(--bg-surface-subtle)] text-[var(--text-faint)]"}`}>{h.status}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <Link href="/hr?tab=behavior" className="inline-flex items-center gap-2 h-10 px-4 rounded-xl bg-[var(--bg-inverted)] text-[var(--text-inverted)] text-[12.5px] font-semibold hover:opacity-90 transition-opacity">
          Assess in HR › Behavior <ArrowRightIcon size={13} className="rtl:rotate-180" />
        </Link>
        {canConfigurePosition && positionId && (
          <button type="button" onClick={() => setConfigOpen(true)} className="ms-2 inline-flex items-center gap-1.5 h-10 px-3 rounded-xl border border-[var(--border-subtle)] text-[12px] text-[var(--text-dim)] hover:text-[var(--text-primary)] transition-colors">
            <CogIcon size={13} /> Configure position behaviors
          </button>
        )}
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-[12px] text-[var(--text-dim)] leading-snug">
        Optional baseline. Anything you score here is saved as the new hire&apos;s first <strong>baseline</strong> assessment — leave it empty to create the employee without one.
      </p>
      ...
      {summary.criticalAlerts.length > 0 && (
        <div className="flex items-start gap-2 rounded-xl border border-rose-500/40 bg-rose-500/[0.08] px-3.5 py-2.5">
          ...
          <p className="text-[12px] text-rose-600 dark:text-rose-300">
            {summary.criticalGaps} critical behavior{summary.criticalGaps > 1 ? "s are" : " is"} below requirement — Attention Required. A high overall score does not clear this.
          </p>
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between gap-2 mb-2">
          <h4 className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Position Behaviors</h4>
          {canConfigurePosition && positionId && (
            <button type="button" onClick={() => setConfigOpen(true)} className="flex items-center gap-1.5 text-[11.5px] text-[var(--text-dim)] hover:text-[var(--text-primary)] transition-colors">
              <CogIcon size={12} /> Configure position behaviors
            </button>
          )}
        </div>
        {!positionId ? (
          <p className="rounded-xl bg-[var(--bg-surface-subtle)]/60 px-4 py-6 text-center text-[12.5px] text-[var(--text-faint)]">Select a position — its expected behaviors load here.</p>
        ) : requirements.length === 0 ? (
          <p className="rounded-xl bg-[var(--bg-surface-subtle)]/60 px-4 py-6 text-center text-[12.5px] text-[var(--text-faint)]">
            This position has no configured behaviors yet.{canConfigurePosition && " Use “Configure position behaviors” to define them."}
          </p>
        ) : (
              ...
              return (
                <div key={catId} className="rounded-xl border border-[var(--border-subtle)] overflow-hidden">
                  <button type="button" onClick={() => toggleCat(catId)} aria-expanded={open}
                    ...
                    <span className="text-[12.5px] font-semibold text-[var(--text-primary)] truncate">{cat?.name ?? "Other"}</span>
                    <span className="flex items-center gap-2 shrink-0 text-[10.5px] text-[var(--text-faint)]">
                      ...
                      {gaps > 0 && crit === 0 && <span className="rounded-full bg-amber-500/12 px-1.5 py-0.5 font-semibold text-amber-600 dark:text-amber-400">{gaps} gap{gaps > 1 ? "s" : ""}</span>}
                          ...
                          <div key={r.behavior_indicator_id} className="px-3.5 py-2.5">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                              <div className="sm:w-[240px] shrink-0 min-w-0">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  {req?.is_critical && <span className="shrink-0 rounded bg-rose-500/12 px-1 py-px text-[9px] font-bold uppercase text-rose-600 dark:text-rose-400">Critical</span>}
                                  {req?.is_mandatory && <span className="shrink-0 rounded bg-amber-500/12 px-1 py-px text-[9px] font-bold uppercase text-amber-700 dark:text-amber-400">Req</span>}
                                </div>
                                <div className="text-[10.5px] text-[var(--text-faint)]">Required: {req?.required_score ?? "—"}</div>
                              </div>
                            ...
                            {critGap && (
                              <input value={r.comment ?? ""} onChange={(e) => setComment(r.behavior_indicator_id, e.target.value)}
                                placeholder="Justification required for critical gap…"
      ...
      <div>
        <div className="flex items-center justify-between gap-2 mb-2">
          <h4 className="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Additional Behaviors</h4>
          <button type="button" onClick={() => setPickerOpen(true)} className="flex items-center gap-1.5 text-[11.5px] text-[var(--text-dim)] hover:text-[var(--text-primary)] transition-colors">
            <span className="w-5 h-5 rounded-full bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] flex items-center justify-center"><PlusIcon size={10} /></span> Add indicator
          </button>
        </div>
        {additionalRows.length === 0 ? (
          <p className="text-[12px] text-[var(--text-faint)]">None. Add indicators beyond the position requirements if relevant.</p>
        ) : (
          <div className="rounded-xl border border-[var(--border-subtle)] divide-y divide-[var(--border-faint)]">
            {additionalRows.map((r) => {
              ...
              return (
                <div key={r.behavior_indicator_id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 px-3.5 py-2.5">
                  ...
                  <BehaviorSlider value={r.employee_score} onChange={(v) => setScore(r.behavior_indicator_id, v)} label={ind?.name ?? "indicator"} />
                  <button type="button" onClick={() => removeRow(r.behavior_indicator_id)} aria-label="Remove"
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <SummaryCard label="Overall Behavior" value={summary.overallScore == null ? "—" : String(summary.overallScore)} />
          <SummaryCard label="Position Match" value={summary.matchPct == null ? "—" : `${summary.matchPct}%`} tone={summary.matchPct == null ? undefined : summary.matchPct >= 100 ? "good" : summary.matchPct < 70 ? "warn" : undefined} />
          <SummaryCard label="Required Avg" value={summary.requiredScore == null ? "—" : String(summary.requiredScore)} />
          <SummaryCard label="Critical Gaps" value={String(summary.criticalGaps)} tone={summary.criticalGaps > 0 ? "bad" : "good"} />
          ...
          <SummaryCard label="Strongest" value={summary.strongestCategoryId ? (catById.get(summary.strongestCategoryId)?.name ?? "—").split(" ")[0] : "—"} />
        </div>
      )}
