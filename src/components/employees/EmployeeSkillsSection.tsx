...
const SKILL_LEVEL_KEY: Record<string, string> = {
  Master: "hr.sk.lvMaster",
  Expert: "hr.sk.lvExpert",
  Advanced: "hr.sk.lvAdvanced",
  Intermediate: "hr.sk.lvIntermediate",
  Beginner: "hr.sk.lvBeginner",
  "No Experience": "hr.sk.lvNoExperience",
};
...
function SkillSlider({
  ...
  const v = value ?? 0;
  const levelLabel = t(SKILL_LEVEL_KEY[levelForScore(v)] ?? "hr.sk.lvNoExperience");
  return (
    <div className="flex items-center gap-3 min-w-0 flex-1">
      <input
        ...
        aria-valuetext={value == null ? t("hr.sk.notAssessed") : `${v} — ${levelLabel}`}
      ...
      <span className={`w-[86px] shrink-0 text-[10.5px] font-medium ${
        ...
        {value == null ? t("hr.sk.notAssessed") : levelLabel}
      </span>
    </div>
  );
}
...
function GapBadge({ row }: { row: ScorableSkill }) {
  ...
  const label = g === "meets" ? t("hr.sk.meets") : g === "below" ? t("hr.sk.gap") : t("hr.sk.notAssessed");
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        {!positionId ? (
          <p className="rounded-xl bg-[var(--bg-surface-subtle)]/60 px-4 py-6 text-center text-[12.5px] text-[var(--text-faint)]">
            {t("hr.sk.selectPositionFirst")}
          </p>
        ) : requirements.length === 0 ? (
          <p className="rounded-xl bg-[var(--bg-surface-subtle)]/60 px-4 py-6 text-center text-[12.5px] text-[var(--text-faint)]">
            {t("hr.sk.noConfigured")}
            {canConfigurePosition && t("hr.sk.noConfiguredHint")}
          </p>
        ) : (
      ...
      {(positionRows.length > 0 || additionalRows.length > 0) && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <SummaryCard label={t("hr.sk.positionScore")} value={summary.positionScore == null ? "—" : String(summary.positionScore)} />
          <SummaryCard label={t("hr.sk.requiredAvg")} value={summary.requiredScore == null ? "—" : String(summary.requiredScore)} />
          <SummaryCard
            label={t("hr.sk.skillMatch")}
          ...
          <SummaryCard label={t("hr.sk.below")} value={String(summary.below)} tone={summary.below > 0 ? "warn" : undefined} />
          <SummaryCard label={t("hr.sk.mandatoryGaps")} value={String(summary.mandatoryGaps)} tone={summary.mandatoryGaps > 0 ? "warn" : "good"} />
        </div>
      )}
      ...
      >
        <div className="p-4 border-b border-[var(--border-subtle)] space-y-2.5">
          <div className="flex items-center justify-between">
            <h3 className="text-[14px] font-bold text-[var(--text-primary)]">{t("hr.sk.addSkillTitle")}</h3>
            <button type="button" onClick={onClose} aria-label={t("hr.sk.close")} className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--text-dim)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)] transition-colors"><CrossIcon size={13} /></button>
          </div>
          <div className="relative">
            ...
            <input
              ...
              placeholder={t("hr.sk.searchSkills")}
          ...
          <select
            ...
            aria-label={t("hr.sk.allCategories")}
...
function PositionSkillsConfig({
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) { setError(json.error || t("hr.sk.saveFailed")); return; }
      onClose();
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-subtle)]">
          <div>
            <h3 className="text-[14px] font-bold text-[var(--text-primary)]">{t("hr.sk.posReqTitle")}</h3>
            <p className="text-[11px] text-[var(--text-dim)] mt-0.5">
              {t("hr.sk.posReqSubtitle")}
            </p>
          </div>
          <button type="button" onClick={onClose} aria-label={t("hr.sk.close")} className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--text-dim)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)] transition-colors"><CrossIcon size={13} /></button>
        </div>
              ...
              <button
                ...
                className={`rounded-full px-2.5 py-1 text-[10.5px] font-semibold transition-colors ${
                  r.is_mandatory
                    ...
                    : "bg-[var(--bg-surface-subtle)] text-[var(--text-faint)]"}`}
              >
                {r.is_mandatory ? t("hr.sk.mandatory") : t("hr.sk.optional")}
              </button>
              <button
                type="button"
                onClick={() => setReqs(reqs.filter((x) => x.skill_id !== r.skill_id))}
                aria-label={t("hr.sk.removeRequirement")}
