    ...
    <section className="bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-subtle)] p-5 md:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
        ...
        {isSuperAdmin && (
          <button
            ...
            onClick={() => alert("Custom-Incoterm editor coming next. v1 ships the 11 ICC 2020 terms + DDU, C&F, CIFFO.")}
          >
            <PlusIcon size={14} />
            Add Custom Incoterm
          </button>
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 gap-2">
          {view.map((row) => (
            <IncotermCard key={row.id} row={row} />
          ))}
          {view.length === 0 && (
...
function IncotermCard({ row }: { row: IncotermRow }) {
  return (
