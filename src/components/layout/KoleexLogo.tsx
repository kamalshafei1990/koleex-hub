/* ---------------------------------------------------------------------------
   KoleexLogo — the KOLEEX wordmark, extracted into its own component so it
   can be reused by the header, the login gate, and anything else that needs
   the brand mark without causing a circular import back through MainHeader.

   Sizing is driven entirely by parent className (h-4, h-6, etc); the viewBox
   keeps the aspect ratio locked.
   --------------------------------------------------------------------------- */

export default function KoleexLogo({ className }: { className?: string }) {
  return (
    <svg
      className={className}
