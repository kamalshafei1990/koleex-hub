...
export default function SourcingSection({
  ...
  const addRole = async () => {
    if (!selProduct) { setAErr("Pick a product"); return; }
  ...
  const removeRole = async (link: Row) => {
    const id = str(link, "id"); if (!confirm("Remove this product sourcing role?")) return;
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2"><NetworkIcon className="h-4 w-4 text-[var(--text-secondary)]" /><h3 className="text-[15px] font-semibold tracking-tight text-[var(--text-primary)]">Sourcing Intelligence</h3></div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => { setCmpRows(null); setPicked(new Set([supplierId])); setCmpOpen(true); }} className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--bg-surface-subtle)] px-3 py-1.5 text-[12px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)]"><ArrowRightLeftIcon className="h-3.5 w-3.5" /> Compare</button>
          <button type="button" onClick={() => { setSelProduct(null); setPq(""); setARole("preferred"); setAErr(null); setAddOpen(true); }} className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--bg-surface-subtle)] px-3 py-1.5 text-[12px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)]"><PlusIcon className="h-3.5 w-3.5" /> Assign role</button>
          <button type="button" onClick={openProfile} className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--bg-surface-subtle)] px-3 py-1.5 text-[12px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)]"><Edit3Icon className="h-3.5 w-3.5" /> Score</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-2xl bg-[var(--bg-surface-subtle)] p-4">
          ...
          <div className="mt-1 text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-faint)]">Sourcing score</div>
        </div>
        <div className="rounded-2xl bg-[var(--bg-surface-subtle)] p-4"><div className="text-2xl font-semibold tracking-tight text-[var(--text-primary)]">{sourcing?.priority ?? "—"}</div><div className="mt-0.5 text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-faint)]">Priority</div></div>
        <div className="rounded-2xl bg-[var(--bg-surface-subtle)] p-4"><div className="text-2xl font-semibold tracking-tight text-[var(--text-primary)]">{sourcing?.preferredProducts ?? 0}</div><div className="mt-0.5 text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-faint)]">Preferred products</div></div>
        <div className="rounded-2xl bg-[var(--bg-surface-subtle)] p-4"><div className="text-2xl font-semibold tracking-tight text-[var(--text-primary)]">{sourcing?.blockedProducts ?? 0}</div><div className="mt-0.5 text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-faint)]">Blocked products</div></div>
      </div>

      {sourcing?.soleSource ? (
        <div className="flex items-center gap-2 rounded-xl bg-amber-500/[0.08] px-3 py-2 text-[12px] text-amber-300"><TriangleWarningIcon className="h-4 w-4 shrink-0" />Dependency exposure: high reliance with no qualified backup. Consider diversifying.</div>
      ) : null}
      {str(sp, "diversification_note") || str(sp, "sourcing_notes") ? (
        <div className="space-y-1.5 rounded-2xl bg-[var(--bg-surface-subtle)] p-4">
          ...
          {str(sp, "diversification_note") ? <div className="text-[11px] text-[var(--text-faint)]"><span className="uppercase tracking-wide">Diversification:</span> {str(sp, "diversification_note")}</div> : null}
        </div>
      ) : null}

      {/* product sourcing roles */}
      <div className="space-y-2.5">
        <SectionLabel>Product sourcing roles</SectionLabel>
        {links.length === 0 ? (
          <div className="rounded-2xl bg-[var(--bg-surface-subtle)]/50 px-6 py-8 text-center text-sm text-[var(--text-faint)]">No product sourcing roles yet — assign preferred / approved / backup roles per product.</div>
        ) : (
          <div className="space-y-2">
            {links.map((l) => {
              const id = str(l, "id");
              const prod = (l.products as Row | null) ?? {};
              const terms = [str(l, "lead_time_days") && `${str(l, "lead_time_days")}d lead`, str(l, "moq") && `MOQ ${str(l, "moq")}`, str(l, "target_price") && `≤ ${str(l, "target_price")}`, str(l, "quality_level") && `${str(l, "quality_level")} quality`].filter(Boolean) as string[];
              return (
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        <span className="truncate text-[13px] font-semibold text-[var(--text-primary)]">{str(prod, "product_name") || "Product"}</span>
                      </div>
                    ...
                    <div className="flex shrink-0 items-center gap-0.5">
                      ...
                      <button type="button" disabled={busyId === id} onClick={() => removeRole(l)} className="rounded-md p-1.5 text-[var(--text-faint)] hover:bg-[var(--bg-surface)] hover:text-rose-400 disabled:opacity-40" title="Remove role"><TrashIcon className="h-3.5 w-3.5" /></button>
                    </div>
                  </div>
                </div>
              );
      ...
      {specializations.length ? (
        <div className="space-y-2.5">
          <SectionLabel>Category specializations</SectionLabel>
          <div className="flex flex-wrap gap-1.5">
            {specializations.map((s) => (
              <span key={str(s, "id")} className="inline-flex items-center gap-1 rounded-full bg-[var(--bg-surface-subtle)] px-2.5 py-1 text-[11px] font-medium text-[var(--text-secondary)]">
                <AwardIcon className="h-3 w-3 text-[var(--text-faint)]" />{str(s, "category_label")}{str(s, "strength_score") ? ` · ${str(s, "strength_score")}` : ""}{s.is_primary ? " · primary" : ""}
              </span>
            ))}
          </div>
        </div>
      ...
      {pEdit ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !pBusy && setPEdit(false)}>
          <div className="w-full max-w-md space-y-4 rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2"><GaugeIcon className="h-4 w-4 text-[var(--text-secondary)]" /><span className="text-[14px] font-semibold text-[var(--text-primary)]">Sourcing score</span></div>
            <div className="text-[11px] text-[var(--text-faint)]">Auto score (from risk · readiness · negotiation · certs): <span className="font-semibold text-[var(--text-secondary)]">{sourcing?.score ?? "—"}</span>. Set an override to pin it.</div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Score override (0–100)"><input type="number" min={0} max={100} className={inputCls} value={pOverride} onChange={(e) => setPOverride(e.target.value)} placeholder="auto" /></Field>
              <Field label="Priority"><input type="number" className={inputCls} value={pPriority} onChange={(e) => setPPriority(e.target.value)} /></Field>
            </div>
            <Field label="Sourcing notes"><textarea className={`${inputCls} min-h-[56px]`} value={pNotes} onChange={(e) => setPNotes(e.target.value)} /></Field>
            <Field label="Diversification note"><input className={inputCls} value={pDiv} onChange={(e) => setPDiv(e.target.value)} placeholder="e.g. need a 2nd source outside CN" /></Field>
            <div className="flex items-center gap-3">
              <button type="button" disabled={pBusy} onClick={saveProfile} className="rounded-lg bg-[var(--bg-inverted)] px-4 py-2 text-[12px] font-semibold text-[var(--text-inverted)] hover:opacity-90 disabled:opacity-50">{pBusy ? "Saving…" : "Save"}</button>
              <button type="button" onClick={() => setPEdit(false)} className="text-[12px] text-[var(--text-faint)] hover:text-[var(--text-secondary)]">Cancel</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {addOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !aBusy && setAddOpen(false)}>
          <div className="max-h-[88vh] w-full max-w-md space-y-4 overflow-auto rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2"><PackageIcon className="h-4 w-4 text-[var(--text-secondary)]" /><span className="text-[14px] font-semibold text-[var(--text-primary)]">Assign product sourcing role</span></div>
            <Field label="Product">
              {selProduct ? (
                <div className="flex items-center justify-between gap-2 rounded-lg bg-[var(--bg-surface-subtle)] px-3 py-2"><span className="text-[12px] text-[var(--text-primary)]">{str(selProduct, "product_name")}</span><button type="button" onClick={() => setSelProduct(null)} className="text-[11px] text-[var(--text-faint)] hover:text-[var(--text-secondary)]">change</button></div>
              ) : (
                <>
                  <input className={inputCls} value={pq} onChange={(e) => setPq(e.target.value)} placeholder="Search products…" />
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Role"><select className={inputCls} value={aRole} onChange={(e) => setARole(e.target.value)}>{SOURCING_ROLE_ORDER.map((r) => <option key={r} value={r}>{SOURCING_ROLE_LABELS[r]}</option>)}</select></Field>
              <Field label="Quality"><select className={inputCls} value={aQuality} onChange={(e) => setAQuality(e.target.value)}>{QUALITY.map((q) => <option key={q} value={q}>{q ? q[0].toUpperCase() + q.slice(1) : "—"}</option>)}</select></Field>
              <Field label="Lead time (days)"><input type="number" className={inputCls} value={aLead} onChange={(e) => setALead(e.target.value)} /></Field>
              <Field label="MOQ"><input className={inputCls} value={aMoq} onChange={(e) => setAMoq(e.target.value)} /></Field>
            </div>
            <Field label="Target price"><input className={inputCls} value={aPrice} onChange={(e) => setAPrice(e.target.value)} placeholder="e.g. ≤ $420/unit" /></Field>
            {aErr ? <div className="text-[12px] text-rose-400">{aErr}</div> : null}
            <div className="flex items-center gap-3">
              <button type="button" disabled={aBusy} onClick={addRole} className="rounded-lg bg-[var(--bg-inverted)] px-4 py-2 text-[12px] font-semibold text-[var(--text-inverted)] hover:opacity-90 disabled:opacity-50">{aBusy ? "Saving…" : "Assign role"}</button>
      ...
      {cmpOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !cmpBusy && setCmpOpen(false)}>
          <div className="max-h-[88vh] w-full max-w-3xl space-y-4 overflow-auto rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2"><ArrowRightLeftIcon className="h-4 w-4 text-[var(--text-secondary)]" /><span className="text-[14px] font-semibold text-[var(--text-primary)]">Compare suppliers</span></div>
            {!cmpRows ? (
              <>
                <div className="text-[11px] text-[var(--text-faint)]">Pick suppliers to compare by sourcing score, risk, certs, negotiation.</div>
                <div className="max-h-56 space-y-1 overflow-auto rounded-lg border border-[var(--border-subtle)] p-2">
                  ...
                  {allSuppliers.length === 0 ? <div className="px-2 py-3 text-center text-[11px] text-[var(--text-faint)]">Loading suppliers…</div> : null}
                </div>
                <div className="flex items-center gap-3">
                  <button type="button" disabled={cmpBusy || picked.size < 1} onClick={runCompare} className="rounded-lg bg-[var(--bg-inverted)] px-4 py-2 text-[12px] font-semibold text-[var(--text-inverted)] hover:opacity-90 disabled:opacity-50">{cmpBusy ? "Comparing…" : `Compare ${picked.size}`}</button>
                  <button type="button" onClick={() => setCmpOpen(false)} className="text-[12px] text-[var(--text-faint)] hover:text-[var(--text-secondary)]">Cancel</button>
                </div>
              </>
            ) : (
              <>
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-[12px]">
                    <thead><tr className="border-b border-[var(--border-subtle)] text-left text-[10px] uppercase tracking-wide text-[var(--text-faint)]">
                      <th className="py-2 pr-3 font-medium">Supplier</th><th className="py-2 pr-3 font-medium">Sourcing</th><th className="py-2 pr-3 font-medium">Risk</th><th className="py-2 pr-3 font-medium">Trust</th><th className="py-2 pr-3 font-medium">Nego</th><th className="py-2 pr-3 font-medium">Certs</th><th className="py-2 pr-3 font-medium">Country</th><th className="py-2 pr-3 font-medium">Pref</th>
                    </tr></thead>
                    <tbody>
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setCmpRows(null)} className="text-[12px] text-[var(--text-faint)] hover:text-[var(--text-secondary)]">← Back</button>
                  <button type="button" onClick={() => setCmpOpen(false)} className="text-[12px] text-[var(--text-faint)] hover:text-[var(--text-secondary)]">Close</button>
                </div>
              </>
            )}
          </div>
