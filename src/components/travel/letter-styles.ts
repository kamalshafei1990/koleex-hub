...
export const LETTER_STYLES = `
  ...
  .inv-a4 {
    ...
    /* THE WATERMARK — the owner's Feiyue letter tiles the company mark
       ...
       Matched to the reference by MEASURING it (~20mm marks, ~11mm row
       pitch) and laid out as a strict GRID: a 2×2 checkerboard tile, KOLEEX
       alternating with the registered Chinese name 科莱恪斯, every mark
       CENTRED in its cell and nothing crossing the tile edge — hand-placed
       coordinates in the first version let row 2's last mark spill past the
       boundary, so every repeat carried a clipped seam. Built from the
       letterhead logo's own SVG paths at 5% black — settled by the owner
       looking at real exports: 4% was barely there, 8% too loud, 12% was a
       mistranslated "decrease". 5% reads as texture, not competition.
