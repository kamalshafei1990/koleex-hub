...
export type SoundDef = {
  ...
  /** Whether the default is ON. THREE TIMES DECIDED, and the owner's first
   *  answer is the one that stands.
   *
   *    2026-09-12  "too many — keep the sounds for basic things"  → four on
   *    2026-09-16  "all sounds are good keep them all and wire them" → all on
   *    2026-09-18  "I said not need alot of sounds in the app just the
   *                 basic is enough"                              → four on
   *
   *  The middle one was read too broadly. He was approving the RECORDINGS —
   *  he had just heard the glass set end to end — and "wire them" meant
   *  every cue should have something to play, which it now does. It did not
   *  mean every cue should play unasked, and turning all thirty on was my
   *  reading, not his instruction. He has now said so twice.
   *
   *  THE RULE BEHIND THE FOUR, so a cue added later knows where it belongs:
   *  a sound is for something you need to know WITHOUT LOOKING AT THE
   *  SCREEN — a call ended, a call failed, a call came back, something went
   *  wrong. Everything you can already see happening stays quiet by default
   *  and waits in Settings → Sounds for whoever wants it. */
  defaultOn: boolean;
};
...
export const SOUND_CATALOG: readonly SoundDef[] = [
  ...
  },
  {
    ...
    notes: READY_TONE,
    /* THE ONE CUE WITH A HISTORY. The owner asked for it off on 2026-09-13
       ("remove the sound of listening") when it was the synthesised tone;
       "all sounds are good keep them all and wire them" (2026-09-16) turns
       it back on with the rest. It is the first one to silence in Settings
       → Sounds if the recorded cue wears the same way the tone did. */
    defaultOn: false,
  },
  {
