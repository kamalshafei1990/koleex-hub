...
export interface ShaderWallpaper {
  ...
  /** THE COLOUR THIS PATTERN IS MEANT TO BE (owner, 2026-08-15: "each
   *  background can have it's default color"). Every shader used to open in
   *  Hub Blue, so picking one showed you the shape and never the intent — a
   *  molten-metal ground arrived cold blue, a terminal arrived un-green. This
   *  is what it opens in when the user has not chosen a colour; the moment
   *  they press a swatch, theirs wins and this is never consulted again. */
  defaultTint?: string;
}
