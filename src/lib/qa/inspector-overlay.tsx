...
/* ---------------------------------------------------------------------------
   The QA inspector's overlay and event engine — split out of inspector.tsx so
   the hover/select machinery (and the route → module table it labels picks
   with) downloads only when someone actually starts inspect mode. The
   provider in inspector.tsx mounts it while active; it was always mounted
   ONLY while active, so nothing about its behaviour changed.
   --------------------------------------------------------------------------- */

import { useEffect, useRef, useState } from "react";
import { usePathname } from "next/navigation";
import { moduleForRoute } from "@/lib/qa/types";
import type { PickedComponent } from "./inspector";
...
export default function InspectorOverlay({ onFinish }: { onFinish: (c: PickedComponent | null) => void }) {
