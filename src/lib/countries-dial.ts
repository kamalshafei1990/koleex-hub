/* Every country, A→Z, with its international dial code.

   WHY THIS IS NOT `COUNTRIES` FROM types/product-form. That list is the
   PRICING list: 29 markets, deliberately ordered by business priority (Egypt,
   then the Gulf, then Asia…) and carrying a `currency` because eighteen
   commercial screens sort and segment by it. Reordering it alphabetically to
   suit the sign-in screen would reshuffle Markets, the Price Calculator,
   Commercial Policy and the packing list.

   The sign-in screen has the opposite requirement: it is public, anyone in the
   world can reach it, and a visitor from Spain or Japan could not say where
   they were. So it gets its own list — complete, alphabetical, no currency.

   Names are generated from Intl.DisplayNames rather than typed by hand, and
   are localised at render time by `countryName()` below when the browser can. */

export interface CountryDial {
  code: string;
  /** English name — the fallback when Intl.DisplayNames is unavailable. */
  name: string;
  dial: string;
}
...
/* One Intl.DisplayNames per language, not one per option: building it inside a
   199-row map re-created the formatter 199 times on every render. */
...
/** The country's name in the reader's language, falling back to English. */
export function countryName(c: CountryDial, lang: string): string {
  if (lang === "en") return c.name;
  return namesFor(lang)?.of(c.code) || c.name;
}

/** The list sorted for the reader's alphabet — Arabic sorts in Arabic, Chinese
 *  by pinyin. Cached per language; sorting 199 rows on every keystroke of an
 *  unrelated field is not free. */
const sorted = new Map<string, readonly CountryDial[]>();
export function countriesFor(lang: string): readonly CountryDial[] {
  const hit = sorted.get(lang);
  if (hit) return hit;
  const list =
    lang === "en"
      ? COUNTRY_DIALS /* already A→Z */
      : [...COUNTRY_DIALS].sort((a, b) =>
          countryName(a, lang).localeCompare(countryName(b, lang), lang),
        );
  sorted.set(lang, list);
  return list;
}

export function dialOf(code: string): string {
  return COUNTRY_DIALS.find((c) => c.code === code)?.dial ?? "";
}
