...
type HeadRow = {
  ...
  payroll_only: boolean | string | null; mgmt_only: boolean | string | null; app: string | null; or_team: boolean | string | null;
...
export async function loadCustomHeads(tenantId: string | null, opts: { activeOnly?: boolean } = {}): Promise<CustomTemplateHead[]> {
  let q = supabaseServer.from("work_report_templates")
    .select("key, family:def->>family, icon:def->>icon, cadence:def->>cadence, hr_only:def->hrOnly, team_only:def->teamOnly, office_only:def->officeOnly, payroll_only:def->payrollOnly, mgmt_only:def->mgmtOnly, app:def->>app, or_team:def->orTeam, name:words->name, desc:words->desc, status, version, updated_at")
  ...
  return ((data ?? []) as HeadRow[]).map((r) => ({
    ...
    mgmtOnly: r.mgmt_only === true || r.mgmt_only === "true",
