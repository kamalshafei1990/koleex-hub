...
const DICT = path.join(ROOT, "src/lib/translations/shipping.ts");
...
const { shippingT } = await import(DICT) as {
  shippingT: Record<string, Record<string, string>>;
};
const entries = new Map(Object.entries(shippingT));
if (entries.size === 0) {
