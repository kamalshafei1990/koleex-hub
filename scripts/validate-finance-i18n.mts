#!/usr/bin/env tsx
/* validate:finance-i18n
 *
 * WHY THIS EXISTS. src/lib/translations/finance.ts had grown to 1,637 keys x 3
 * languages — 287 KB of source, 200 KB minified — and all 33 finance
 * components imported the whole thing. Measured 17/09/2026: that one chunk sat
 * in 28 of the 29 finance routes and nowhere else in the Hub, which is why
 * every finance screen weighed 800-1,010 KB. /finance/orders reads 121 of
 * those keys. /finance/cash-flow reads twelve.
 *
 * The strings now live one file per namespace under translations/finance/ and
 * each screen imports only what it uses. Two ways that silently comes undone:
 *   · someone imports the `financeT` union again — 287 KB back, nothing looks
 ...
 *
 * Run: npm run validate:finance-i18n
 */
import fs from "node:fs";
import path from "node:path";

const ROOT = process.cwd();
const NS_DIR = "src/lib/translations/finance";
const COMPONENTS = "src/components/finance";
...
const nsFiles = fs.readdirSync(path.join(ROOT, NS_DIR))
  .filter((f) => f.endsWith(".ts") && f !== "account-names.ts")
  .map((f) => f.slice(0, -3));
...
const files = walk(COMPONENTS).filter((f) => fs.readFileSync(path.join(ROOT, f), "utf8").includes("translations/finance"));

console.log(`\nA. The split holds — ${nsFiles.length} namespaces, ${owner.size} keys, ${files.length} finance components`);
dupes.length
...
for (const f of files) {
  ...
  if (/^\s*import\s*\{[^}]*\bfinanceT\b/m.test(src)) {
    fail(`${name} imports the financeT union`, "287 KB back on this route. Import the namespaces it reads from translations/finance/<ns>.");
    continue;
  }
  const imported = new Set([...src.matchAll(/from "@\/lib\/translations\/finance\/([a-zA-Z-]+)"/g)].map((m) => m[1]));
...
console.log(failures === 0 ? "\n✓ finance i18n: all checks passed" : `\n✗ ${failures} failed`);
