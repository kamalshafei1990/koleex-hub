...
/* ===========================================================================
   ...
   view-as(role), account-switch, concurrent, db-error.

   No real passwords, no production data, no secrets. Pure in-memory fakes.
   Structured around a main() so node:test mock.module
   composes with the native loader.
   Run: NODE_OPTIONS=--conditions=react-server \
        tsx --experimental-test-module-mocks scripts/validate-auth-equivalence.ts
   (the mocks flag is not accepted inside NODE_OPTIONS; tsx passes it to node)
   ========================================================================== */
...
async function main() {
  ...
  /* Detect whether cache() memoizes outside a request scope. If getServerAuth()
     returns a STALE result when STATE changes between calls, cross-scenario
     comparison on the cached path is unsound; we adapt honestly. */
  ...
  const CACHE_ACTIVE = probe1?.account_id === probe2?.account_id;
  ...
  console.log(`auth-equivalence  (cache-active-outside-request=${CACHE_ACTIVE})\n`);
  ...
  await equiv("view-as(account) → target context, viewing_as, real=sa", (c) => {
    ...
    assert.equal(c.is_super_admin, false);
  });
